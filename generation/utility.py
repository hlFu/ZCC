...
class utility:
    ...
    def newMap(self,funcName,space=32,reserve=0):
        ...
        for v in global_context.local[funcName].compound_statement.context.local:
            s_class=global_context.local[funcName].compound_statement.context.local[v].storage_class
            Type=global_context.local[funcName].compound_statement.context.local[v].type
            size=global_context.local[funcName].compound_statement.context.local[v].size
            if(s_class == 'static'):
                v=v+'%.d' % self.magicNum
                self.magicNum+=4
